...
// ----------------------------------------------------
        ...
        const handleForm = async (event) => {
            ...
            try {
                ...
                const response = await axios.post(`${URL}/user/transactions`,formData ,options)
                const newTransaction = response.data.transaction // new added
                console.log(newTransaction, "Posted New")
                setAllTransactions(prev => [newTransaction, ...prev])
                onClose() // Show From Dashboard
                onClose() // Show From Transactions
        ...
        return (
            <div className="top-0 -left-2 md:left-0  z-50 fixed h-screen w-screen bg-linear-to-r from-slate-900/60 to-blue-800/60 backdrop-blur-xl flex flex-col justify-center items-center">
                <div className="w-[90%] md:w-2/3 lg:w-1/3 px-5 md:px-8 py-4 md:py-6 bg-slate-500 rounded-lg shadow-xl flex flex-col justify-center items-center">
                    <form id="add-transaction-form" onSubmit={handleForm} className="w-full">
                        {/* Title */}
                        ...
                        <section className="w-full flex flex-col justify-center items-start mb-4">
                            ...
                            className="h-10 w-full text-sm md:text-base bg-slate-200 text-black px-4 rounded-lg outline-none cursor-pointer">
                                <option value="Income" className="text-blue-900 font-semibold">Income</option>
                                <option value="Expense" className="text-blue-900 font-semibold">Expense</option>
                            </select>
                        </section>

                        {/* Category */}
                        <section className="w-full flex flex-col justify-center items-start mb-4">
                            <label htmlFor="category" className="text-white text-base font-semibold mb-2">Category</label>
                        ...
                        {/* Date */}
                        <section className="w-full flex flex-col justify-center items-start mb-4 ">
                            <label htmlFor="date" className="text-white text-base font-semibold mb-2">Date</label>
                        ...
                        <section className="w-full flex flex-col justify-center items-start mb-10">
                            <label htmlFor="note" className="text-white text-base font-semibold mb-2">Note</label>
                        ...
                        className="h-10 md:h-12 w-40 text-sm md:text-base text-white font-semibold bg-gray-600 rounded-lg cursor-pointer
                        hover:bg-gray-700  transition-colors duration-300 mr-5">Cancel</button>

                        <button type="submit"
                        className="h-10 md:h-12 w-40 text-sm md:text-base text-white font-semibold bg-blue-600 rounded-lg flex flex-row justify-center items-center cursor-pointer
